...
MAX_POSTS = 52
POST_NUMBER = 1 # to start numbering the posts from, starting from oldest to newest
POSTS_SUCCESS = 0
POSTS_FAIL = 0
FILES_SUCCESS = 0
FILES_FAIL = 0
...
def downloader(url, destination):
    try:
        ...
        if response.status_code == 200:
            with open(destination + ".jpg", 'wb') as file:
...
def url_ToMediaItems(driver, post_page_url, folder_path):
    ...
    if (len(driver.find_elements(By.XPATH, '//*[@class="swiper-wrapper carousel"]')) == 0):
        ...
        try:
            ...
            POSTS_SUCCESS += 1
            FILES_SUCCESS += 1
        # if the item is a video tag, skip
        except common.NoSuchElementException:
            try:
                parent_div.find_element(By.TAG_NAME, "video").get_attribute('src')
                messages(f"video skipped, post number: {POST_NUMBER}","e")
                VIDEO_SKIPPED += 1
                #POST_NUMBER -= 1
        ...
        try:
            ...
            for media_item in media_items_list:
                ...
                try:
                    ...
                    driver.switch_to.window(profile_tab_handle)
            # after the for loop
            if (FILES_FAIL == len(media_items_list) or video_in_post_skipped == len(media_items_list)):
                POSTS_FAIL += 1
                POST_NUMBER -= 1
            else:
                POSTS_SUCCESS += 1
...
def InstaGrab():
    ...
    for url in reversed(posts_list):
        try:
            ...
            url_ToMediaItems(driver, url, folder_path + "/" + str(POST_NUMBER))
        except Exception as e:
    ...
    messages("#-#-# Done! #-#-#", "b")
    messages(
        f"Succeded posts: {POSTS_SUCCESS}, Failed posts: {POSTS_FAIL}, Succeeded files: {FILES_SUCCESS}, Failed files: {FILES_FAIL}, Skipped Videos: {VIDEO_SKIPPED}",
        "cy")
    driver.quit()
    sleep(100)
